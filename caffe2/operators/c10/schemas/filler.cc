...
struct DTypeParameter final {
    static int parse(const caffe2::ArgumentHelper& helper) {
        auto dtype = helper.GetSingleArgument<int>("dtype", caffe2::TensorProto_DataType_FLOAT);
        if (!helper.HasArgument("dtype") &&
            helper.HasArgument("value")) {
...
struct ValueParameter final {
    static float parse(const caffe2::ArgumentHelper& helper) {
        return helper.GetSingleArgument<float>("value", 0);
    }
};
...
struct ValuesParameter final {
    static Tensor<CPUContext> parse(const caffe2::ArgumentHelper& helper) {
...
private:
    ...
    static Tensor<CPUContext> ExtractValues(const caffe2::ArgumentHelper& helper) {
        auto source_values = helper.GetRepeatedArgument<Type>("values");
        Tensor<CPUContext> values;
...
namespace caffe2 {
    ...
    REGISTER_C10_OPERATOR_FOR_CAFFE2_DISPATCH_WITH_ARRAY_INPUT_AND_PARAMETERS(ops::GivenTensorFill<int64_t>, void, C10GivenTensorInt64Fill_DontUseThisOpYet, ShapeParameter, ExtraShapeParameter, InputAsShapeParameter, ValuesParameter<int64_t>)
}
