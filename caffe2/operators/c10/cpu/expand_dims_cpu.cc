#include "caffe2/operators/c10/schemas/expand_dims.h"
...
void expand_dims_op_cpu_impl(const Tensor<CPUContext>& input, Tensor<CPUContext> *output, const std::vector<int>& dims, caffe2::ops::ExpandDims::State* state, CPUContext* context) {
    if (!state->initialized) {
        ...
        state->dims.erase(std::unique(state->dims.begin(), state->dims.end()), state->dims.end());
        if (state->dims.size() < originalSize) {
            LOG(WARNING) << "Parameter `dims` has repeated dimensions.";
        }
        CAFFE_ENFORCE(state->dims.front() >= 0, "Dimension ids must be non-negative.");
...
namespace c10 {
    C10_REGISTER_KERNEL(caffe2::ops::ExpandDims)
        .kernel(&expand_dims_op_cpu_impl<float>)
