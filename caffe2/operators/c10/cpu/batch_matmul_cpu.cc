...
void batch_matmul_op_cpu_impl(Tensor<CPUContext> A, Tensor<CPUContext> B, Tensor<CPUContext>* Y, int trans_a, int trans_b, int broadcast, int use_scratch, caffe2::ops::BatchMatmul::State* state, CPUContext* context) {
    ...
    if (use_scratch && state->scratch.get() == nullptr) {
        state->scratch = std::make_shared<Tensor<CPUContext>>();
    }
    ...
    if (ndims_A == 1 && ndims_B == 1) {
        ...
        math::Dot<T, CPUContext>(
                dims_A[0], data_A, data_B, Y->template mutable_data<T>(), context);
    } else {
        ...
        for (size_t p = 0; p < num_outer_batches; ++p) {
            math::GemmBatched<T, CPUContext, Engine>(
                    ...
                    Y_data + p * Y_stride,
                    context,
                    use_scratch ? state->scratch.get() : nullptr);
        }
    }
}
} // namespace
...
namespace c10 {
    C10_REGISTER_KERNEL(caffe2::ops::BatchMatmul)
        .kernel(&batch_matmul_op_cpu_impl<float>)
